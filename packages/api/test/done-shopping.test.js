...
describe('Done Shopping', () => {
  ...
  // Batch is how many times a Recipe is being made on this trip, so it ends with the trip, the way
  // it does when one Recipe is taken off the list on its own. Otherwise the next week's first Add
  // would inherit a doubling nobody asked for this week.
  it('puts every Batch back to 1 along with the flag', async (t) => {
    const app = await startApp(t);
    ...
    assert.deepEqual(
      (await readRecipes(app)).map((r) => [r.name, r.selected, r.batch]),
      [['Minestrone', false, 1]],
    );
  });
